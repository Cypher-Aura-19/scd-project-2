...
router.get("/total-review", async (req, res) => {
  try {
    const reviews = await Reviews.find({})
      .populate('productId', 'name') // Populate the 'productId' with the 'name' field
      .exec();

    // If you want to send the total number of reviews along with the product name
    const totalReviews = reviews.length;
    res.status(200).send({ totalReviews, reviews });
  } catch (error) {
...
router.get("/:userId", async (req, res) => {
  ...
  try {
    const reviews = await Reviews.find({ userId: userId }).populate('productId', 'name').sort({ createdAt: -1 });
